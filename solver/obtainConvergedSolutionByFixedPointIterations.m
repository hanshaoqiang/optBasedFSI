...
fFSI_prev_fp = fFSI_fp;
% Estimation of initial displacements
disp_fsi_prev_fixed_iter    = 2.5*dFSIVec(:,3) - 2*dFSIVec(:,2) + 0.5*dFSIVec(:,1);
% initial update
cord_fsi_update_prev_iter = zeros(length(fFSI_fp),1);

fsiVel_prevTime             = str_vel(1:2*length(mesh_s.nodes));
fsiVel_prevTime             = fsiVel_prevTime(mesh_s.fsiDOF)';
vel_fsi_prev_fixed_iter     = fsiVel_prevTime';
currentResidual             = 10;
...
while(currentResidual>fsiProp.fsiResidual && iterCount < fsiProp.maxFpIterations)
    ...
    %%% Calculating the residual and printing it
    %%% Here the objectionFunction is nothing but the difference between
    %%% the velocities of 
    currentResidual = norm( (cord_fsi_update - cord_fsi_update_prev_iter) ,inf);
    ...
    mesh_f.elemOrder = 3;
    FluidTitle = ['Fluid_Result_at_Time_',num2str(transient.i),'_',num2str(iterCount),'.vtk'];
    writeResultToVtk(mesh_f, up_fp, FluidTitle);
    %%% Writing Structure Result
    mesh_s_w.elemOrder = 3;
    StructureTitle = ['Structure_Result_at_Time_',num2str(transient.i),'_', num2str(iterCount),'.vtk'];
    ...
    mesh_f.elemOrder = 3;
    FluidMeshDispTitle = ['Fluid_Mesh_Disp_Result_at_Time_',num2str(transient.i),'_',num2str(iterCount),'.vtk'];
    ...
    fFSI_prev_fp    = fFSI_fp;
    cord_fsi_update_prev_iter   = cord_fsi_update;
