...
%                                                                         %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function M = computeIGAMassMatrixVectorTransport2D(p,U,q,V,CP,int)
%% Function documentation
%
% Returns the master mass matrix for a vector convection-diffusion-reaction 
% problem in 2D.
%
%         Input : 
        ...
        if (U(i+1)~=U(i) && V(j+1)~=V(j))
            ...
            for kv = 1:length(GPv)
                for ku =1:length(GPu)
                    ...
                    % Form the basis function matrix:
                    Rmatrix = zeros(2,ndof_e);

                    % Loop over all the entries of the basis functions matrix
                    for rCounter=1:ne
                        Rmatrix(1,2*rCounter-1) = R(rCounter);
                        Rmatrix(2,2*rCounter) = R(rCounter);
                    end
