...
%                                                                         %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function index = plot_steadyStateVectorTransportProblem2D(p,q,U,V,CP,rb,prb,F,transportVector,component,graph)
...
[Xp,Yp,Zp] = createBSplineSurface(p,q,U,V,CP,150,150);

% Compute point coordinates for the deformed geometry, the supports and the
% force arrows
[Xp_def,Yp_def,Zp_def] = createBSplineSurface(p,q,U,V,CPd,150,150);
[xs_def,ys_def,zs_def] = createSupports2D(CPd,rb);
[pxs,pys,pzs] = createSupports2D(CP,prb);
