...
for c_element=1:size(mesh.elements(:,1))
    ...
    if(component == 4)
        for j=1:3
            u(j) = up( EFT(1,3*j-2) );
            v(j) = up( EFT(1,3*j-1) );
            scalarElement(j) = sqrt(u(j)*u(j) + v(j)*v(j));
        end
    end
    ...
    graph.h = surf(P(:,:,1),P(:,:,2),P(:,:,3),scalarOverDomain(:,:,1),'EdgeColor','none');
    hold on;
end
...
% Displaying the arrows for velocity direction and magnitude
if(component == 4)
   quiver(mesh.nodes(:,1), mesh.nodes(:,2), up(1:3:end), up(2:3:end),'k'); 
end
